...
const CATEGORY = {
  news: { label: 'ニュース', cls: 'cat-news' },
  guide: { label: '入門ガイド', cls: 'cat-guide' },
  tools: { label: 'ツール', cls: 'cat-tools' },
};
...
// ---- 一覧ページ ----
function listPage(items, heading, lead) {
  const body = `<section class="list-page">
  <h1>${esc(heading)}</h1>
  <p class="lead">${esc(lead)}</p>
  <div class="grid">${items.map(a => card(a)).join('\n')}</div>
</section>`;
  return layout({ title: `${heading} | ${site.title}`, description: lead, pageUrl: url(heading === 'ニュース一覧' ? 'news' : 'guides'), body });
}
...
${recent.length ? `<section><h2 class="sec-title">最近のニュース</h2><div class="grid">${recent.map(a => card(a)).join('\n')}</div><p class="more"><a href="./news.html">ニュース一覧へ →</a></p></section>` : ''}
${guides.length ? `<section><h2 class="sec-title">入門ガイド・ツール</h2><div class="grid">${guides.map(a => card(a)).join('\n')}</div><p class="more"><a href="./guides.html">ガイド一覧へ →</a></p></section>` : ''}`;
...
writeFileSync(join(DIST, 'guides.html'), listPage(articles.filter(a => a.category !== 'news'), '入門ガイド・ツール一覧', 'はじめての人向けのやさしい解説とツール紹介です。'));
